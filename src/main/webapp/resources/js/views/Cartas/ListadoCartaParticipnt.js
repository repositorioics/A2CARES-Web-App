            ...
            $.getJSON(dir.GetCartasParticipanteUrl,{parametro : $('#parametro').val(), ajax : 'true' },function(data){
                ...
                if(len == 0){
                    ...
                    for ( var i = 0; i < len; i++) {
                        ...
                        var extension =     data[i].tieneExtesion == true ? '<a class="dropdown-item2 btnExtension" href="'+ partsUrl +'"><i class="fas fa-file-excel text-success" aria-hidden="true"></i> Extensión <span class="badge badge-primary" style="border-radius: 5px"> '+ data[i].cantidadExtension +' </span> </a>':'';
                        ...
                        var permisoEditParte = (parametros.isAuthorizeAny == 'true') ? '<a class="dropdown-item2 btnViewParte" style="cursor: pointer"  data-id="' + data[i].idParticipanteCarta + '"><i class="fas fa-sync text-warning"></i> Modificar Partes</a> <div class="dropdown-divider"></div>': '<a hidden="hidden" class="dropdown-item2"><i class="fas fa-sync text-warning"></i> Modificar Partes</a>';
                        var permisoDeleteAll = (parametros.isAuthorizeAny == 'true') ? '<a class="dropdown-item2 btnDelete" style="cursor: pointer" data-id="' + data[i].idParticipanteCarta + '"><i class="fas fa-user-times text-danger" aria-hidden="true"></i> Anular</a> <div class="dropdown-divider"></div>' : '<a  class="dropdown-item2 btnDelete" hidden="hidden" ><i class="fas fa-user-times text-danger" aria-hidden="true"></i> Anular</a>';
                        ...
                        var dropdown  = (data[i].anulada == false) ? '<div class="btn-group">'+
                            ...
                            '<a class="dropdown-item2 btnEditar" href='+ editarUrl +' data-id="' + data[i].idParticipanteCarta + '"><i class="fas fa-user-edit text-warning"></i> Editar Carta</a>' +
