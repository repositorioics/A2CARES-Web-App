...
public class ControlAsistenciaService {
    ...
    public List<ReporteHorasICSDto> getReporteHorasSitiosDeptoICS(FiltroMxEnfermoDto filtro,String idDepto,String idSitio)throws Exception{
        Session session = sessionFactory.getCurrentSession();

        Query query = session.createSQLQuery("call sp_imprimir_reporte_diario_horas_ics_depto_sitios(:fechaInicio, :fechaFin,:idSitio,:idDepto)");

        if (filtro.getFechaInicio() != null && filtro.getFechaFin() != null) {
            query.setParameter("fechaInicio", filtro.getFechaInicio());
            query.setParameter("fechaFin", filtro.getFechaFin());
            query.setParameter("idSitio", idSitio);
            query.setParameter("idDepto", idDepto);
        }
        query.setResultTransformer(Transformers.aliasToBean(ReporteHorasICSDto.class));
        return query.list();
    }
