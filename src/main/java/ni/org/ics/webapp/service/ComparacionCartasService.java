...
public class ComparacionCartasService {
    ...
    public List<ComparacionRelFamCartasDto> getDiferenciasRelFam(){
        ...
        Query query = session.createSQLQuery("select t1.CODIGO_PARTICIPANTE as codigoParticipante, DATE_FORMAT(t1.FECHA_FIRMA, '%d-%m-%Y') as fechaFirma, "+
                ...
                "t1.NOMBRE1_TUTOR as nombre1TutorC, t1.NOMBRE2_TUTOR as nombre2TutorC, t1.APELLIDO1_TUTOR as apellido1TutorC, t1.APELLIDO2_TUTOR as apellido2TutorC, "+
                ...
                "(select m.es from mensajes m where m.catRoot = 'CAT_RF_TUTOR' and m.catKey = t1.RELACION_FAMILIAR) as relacionFamiliarC, " +
                "(select m.es from mensajes m where m.catRoot = 'CAT_RF_TUTOR' and m.catKey = CAST(t2.RELACION_FAMILIAR AS CHAR)) as relacionFamiliarS "+
                "from cartas_consentimientos t1 inner join scan_participante_carta t2 on t1.CODIGO_PARTICIPANTE = t2.CODIGO_PARTICIPANTE "+
                "inner join participantes t3 on t1.CODIGO_PARTICIPANTE = t3.CODIGO "+
                ...
                "or (t1.RELACION_FAMILIAR is null or t2.RELACION_FAMILIAR is null or t1.RELACION_FAMILIAR != t2.RELACION_FAMILIAR) order by t1.CODIGO_PARTICIPANTE"
        );
