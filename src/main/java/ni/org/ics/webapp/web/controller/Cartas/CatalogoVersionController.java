...
import ni.org.ics.webapp.domain.scancarta.Extensiones;
...
public class CatalogoVersionController {
    ...
    //region todo Versión de la Cartas
...
//endregion **fin Partes


    //region todo ** EXTENSION DE LA CARTA **
    ...
    public String getAllExtensiones(Model model)throws Exception{
        try{
            ...
            model.addAttribute("version",version);
            model.addAttribute("editando", false);
            List<Extensiones>listAllExtensiones = this.scanCartaService.getAllExtension();
            model.addAttribute("listAllExtensiones",listAllExtensiones);
    ...
    public ResponseEntity<String>saveOrupdate(@RequestParam("idextension")Integer idextension
                                             ...
                                             ,@RequestParam(value = "activo", defaultValue = "", required = false)String activo
                                             ,@RequestParam("editando")String editando)throws Exception{
        try{
            Version v = new Version();
            String nameComputer = InetAddress.getLocalHost().getHostName();
            boolean estado = activo.equals("on")? true:false;
    ...
    @RequestMapping(value = "activarExtension", method = RequestMethod.POST, produces = "application/json")
    public ResponseEntity<String> activarExtension(@RequestParam("idextension") Integer idextension)throws Exception{
        try{
            Extensiones xtension = this.scanCartaService.getExtensionById(idextension);
            if (xtension!=null){
                xtension.setActivo(true);
                this.scanCartaService.saveORupdateExtension(xtension);
            }
            return JsonUtil.createJsonResponse(xtension);
        }catch (Exception e){
            System.out.println(e.getMessage());
            Gson gson = new Gson();
            String json = gson.toJson(e.toString());
    ...
    //
    @RequestMapping(value = "desactActivarExtension", method = RequestMethod.POST, produces = "application/json")
    public ResponseEntity<String> desactActivarExtension(@RequestParam("idextension") Integer idextension)throws Exception{
        try{
            Extensiones xtension = this.scanCartaService.getExtensionById(idextension);
            if (xtension!=null){
                xtension.setActivo(false);
                this.scanCartaService.saveORupdateExtension(xtension);
            }
            return JsonUtil.createJsonResponse(xtension);
        }catch (Exception e){
    ...
    //endregion ** FIN EXTENSION DE LA CARTA **
