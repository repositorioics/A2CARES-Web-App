...
import ni.org.ics.webapp.dto.BhcEnviarDto;
...
public class SerologiaController {
    ...
    //endregion
    //region Formulario Biometrias
    @RequestMapping(value="/createBhc", method = RequestMethod.GET)
    public String createBhc(ModelMap model) throws Exception{
        try {
            model.addAttribute("agregando",true);
            model.addAttribute("editando",false);
            model.addAttribute("caso", new ParticipanteSeroDto());
            return "/Serologia/BHCForm";
        } catch (Exception e) {
            logger.error(e.getMessage());
            return "404";
        }
    ...
    //region Ir Formulario para Editar
    @RequestMapping(value = "/editMuestraBhc", method = RequestMethod.GET)
    public String editMuestraBhc(Model model, @PathVariable("idbhc") String idbhc) throws Exception
    {
        try{
    ...
    @RequestMapping(value = "/enviarMuestraBhc", method = RequestMethod.POST, produces = "application/json")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody
    ResponseEntity<String> enviarMuestraBhc(@RequestBody BhcEnviarDto muestraForEnvio)throws Exception{
        try{
            BhcEnvio objToSave = new BhcEnvio();
            ...
            String idMxBhc = muestraForEnvio.getIdbhc().toString();
            Bhc objSerologia = this.serologiaService.getBhcById(idMxBhc);
            objSerologia.setIdbhc(objSerologia.getIdbhc());
            this.serologiaService.ModificarEnvioBhc(muestraForEnvio.getIdbhc());//Modifica campo Cerrado
    ...
    @RequestMapping(value = "/sendAllBhc", method = RequestMethod.POST, produces = "application/json")
    @ResponseStatus(HttpStatus.OK)
    public @ResponseBody ResponseEntity<String> sendAllbhc(@RequestParam(value="numenvio", required=false ) Integer numenvio
            ,@RequestParam(value="desde", required=false ) String desde
            ,@RequestParam(value="hasta", required=false ) String hasta
            ,@RequestParam(value="fechaEnvio", required=false ) String fechaEnvio
            ,@RequestParam(value="horaEnvio", required=false ) String horaEnvio
        ...
        try{
            ...
            double temp = Double.parseDouble(temperatura);
            envio.setTemperatura(temp);
            this.serologiaService.save_Envio_Bhc(envio);// aqui guardo los datos del envio
            List<Bhc> ListaSerologiaYaEnviadas = this.serologiaService.ObtenerBhcEnviadas(fdesde,fhasta);// Serologia en pasivo='0'  y cerrado='0'

            if (envio.getIdbhcenvio()!=null){
                if (ListaSerologiaYaEnviadas.size()>0){
                    for (Bhc obj: ListaSerologiaYaEnviadas){
                        Bhc_Detalles_Envio detalles_envio= new Bhc_Detalles_Envio();
                        this.serologiaService.ModificarEnvioBhc(obj.getIdbhc());
                        Bhc objSerologia = this.serologiaService.getBhcById(obj.getIdbhc().toString());
                        detalles_envio.setBhc(objSerologia);
                        detalles_envio.setBhcEnvio(envio);
                        this.serologiaService.save_Detalles_Bhc_Envio(detalles_envio);// guardo el detalle del envio
                       // sero.setEnviado('1');
                    }
...
public ResponseEntity<String>GuardarBhc (@RequestParam(value = "idbhc", required=false, defaultValue="") String idbhc
        ,@RequestParam( value = "idParticipante", required = false, defaultValue=""  ) String idParticipante
        ,@RequestParam( value = "fecha",          required = false, defaultValue=""  ) String fecha
        ,@RequestParam( value = "volumen",        required = false, defaultValue=""  ) String volumen
        ,@RequestParam( value = "observacion",    required = false, defaultValue=""  ) String observacion
    ...
    try{
        ...
        if (  Float.parseFloat(volumen) > 2 || volumen.equals("")){
            Map<String, String> map = new HashMap<String, String>();
            map.put("msj", "Volumen no puede ser Mayor que 2 ml." );
            return JsonUtil.createJsonResponse(map);
        }
        if (fecha.equals("")){
            Map<String, String> map = new HashMap<String, String>();
        ...
        String ComputerIp = InetAddress.getLocalHost().getHostAddress();
        if (tiporequest.equals("false")){//Guardar Nuevo Registro
            if (!serologiaService.ExisteBhc(DateUtil.StringToDate(fecha,"dd/MM/yyyy"),idParticipante)){
                sero.setDeviceid(nameComputer);
                sero.setRecordIp(ComputerIp);
                sero.setEstado('1');
                sero.setPasive('0');
           ...
           //     Integer codeCasa= (casaCHF.equals(""))?0:Integer.parseInt(casaCHF);
            ...
            Serologia objToEdit = this.serologiaService.getSerologiaById(idbhc);
            objToEdit.setDeviceid(nameComputer);
            objToEdit.setRecordIp(ComputerIp);
            objToEdit.setRecordUser(SecurityContextHolder.getContext().getAuthentication().getName());
            objToEdit.setFecha(DateUtil.StringToDate(fecha, "dd/MM/yyyy"));
...
//endregion
    ...
    @RequestMapping(value = "/listBHC", method = RequestMethod.GET)
    public String listBhc(Model model)throws Exception{
        List<MessageResource> numero_envio = messageResourceService.getCatalogo("CAT_ENVIO_SEROLOGIA");
        model.addAttribute("numero_envio", numero_envio);
        ...
        model.addAttribute("sitios", sitios);
        model.addAttribute("sitiosDestino", sitiosDestino);
        return "/Serologia/ListBhc";
    }
    //Serologia/listMuestrasNoEnviadas BHC
    @RequestMapping(value = "/listMuestrasNoEnviadasBhc", method = RequestMethod.GET, produces = "application/json")
    public @ResponseBody List<BhcDto> ListaMuestraToJson() throws ParseException {
    ...
    public @ResponseBody List<SerologiaDto> ListaMuestraBToJson() throws ParseException {
...
*/
    ...
    @RequestMapping(value = "/listEnviosMuestrasBhc", method = RequestMethod.GET, produces = "application/json")
    public String listEnviosMuestrasBhc(Model model)throws Exception{
        /*List<Serologia> serologias = this.serologiaService.SerologiaNoEnviada();
        model.addAttribute("serologias",serologias);
*/
